...
import (
	...
	"github.com/ethersocial/go-ethersocial/graphql"
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(SkynetFlag.Name) && cfg.DataDir == node.DefaultDataDir():
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, LegacyTestnetFlag, RopstenFlag, RinkebyFlag, GoerliFlag, SkynetFlag)
	...
	case ctx.GlobalBool(SkynetFlag.Name):
		...
		setDNSDiscoveryDefaults(cfg, params.KnownDNSNetworks[params.SkynetGenesisHash])
	case ctx.GlobalBool(RinkebyFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 4
		}
