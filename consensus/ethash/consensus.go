...
var (
	ethersocialBlockReward          = big.NewInt(9e+18) // Block reward in wei for successfully mining a block
	ethersocialByzantiumBlockReward = big.NewInt(5e+18) // Block reward in wei for successfully mining a block upward from Byzantium
...
func ethersocialAccumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if config.IsByzantium(header.Number) {
		blockReward = ethersocialByzantiumBlockReward
	}
	// Accumulate the rewards for the miner and any included uncles
	reward := new(big.Int).Set(blockReward)
	r := new(big.Int)
